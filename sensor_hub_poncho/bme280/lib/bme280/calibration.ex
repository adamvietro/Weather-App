defmodule Bme280.Calibration do
  ...
  @temp_press_len 26  # 0x88..0xA1
  @hum_start 0xE1
  @hum_len 7          # 0xE1..0xE7

  @doc """
  Reads all calibration values from the sensor and returns a %Calibration{} struct.
  """
  def read_all(i2c, sensor) do
    # Read temperature & pressure calibration (0x88..0xA1)
    <<t_p::binary-size(@temp_press_len)>> = I2C.write_read!(i2c, sensor, <<@temp_press_start>>, @temp_press_len)

    <<dig_T1_lsb, dig_T1_msb,
      dig_T2_lsb, dig_T2_msb,
      dig_T3_lsb, dig_T3_msb,
      dig_P1_lsb, dig_P1_msb,
      dig_P2_lsb, dig_P2_msb,
      dig_P3_lsb, dig_P3_msb,
      dig_P4_lsb, dig_P4_msb,
      dig_P5_lsb, dig_P5_msb,
      dig_P6_lsb, dig_P6_msb,
      dig_P7_lsb, dig_P7_msb,
      dig_P8_lsb, dig_P8_msb,
      dig_P9_lsb, dig_P9_msb,
      dig_H1>> = :binary.bin_to_list(t_p)

    # Read humidity calibration (0xE1..0xE7)
    <<h::binary-size(@hum_len)>> = I2C.write_read!(i2c, sensor, <<@hum_start>>, @hum_len)
    <<dig_H2_lsb, dig_H2_msb, dig_H3, h4_lsb, h4_msb_bits, h5_msb_bits, dig_H6>> = :binary.bin_to_list(h)
    ...
    %__MODULE__{
      ...
      dig_H5: signed(((h5_msb_bits &&& 0xF0) >>> 4) ||| (h5_msb_bits <<< 4)), # will correct bit shift
